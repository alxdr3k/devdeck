...
### Q-023: When, if ever, should DevDeck become a control surface that executes workflow actions?

- Opened: 2026-05-02
- Owner: project
- Status: future
- Proposed Answer: Keep DevDeck dogfood MVP read-mostly. Record execution as a future control-surface design tension. If execution is added later, DevDeck should request actions through a foreground workflow runner owned by `my-skill` or a successor workflow substrate, then read structured event/result artifacts back into `ProjectStatus` and `AttentionItem`. DevDeck should not directly own shell mutation semantics.
- Blocks: future command execution, future workflow runner integration, post-MVP control surface design
- Resolution: —

**Context**

The user expects that, once DevDeck is useful as a projection layer, the desire to execute obvious next actions from DevDeck will arise. That pressure is real, but accepting execution changes the product from:
...
This is a materially different trust and safety model. Dogfood MVP should validate ranking, source trust, handoff quality, and operator pause before crossing that boundary.

**Design tension**

Execution is valuable because it can reduce context switching and close loops faster. It is risky because `git`, `gh`, test, merge, push, and agent-loop commands have side effects and require approval, replay/audit logs, interruption handling, and recovery semantics.
...
This preserves DevDeck as a cockpit while keeping execution semantics in the workflow substrate.
